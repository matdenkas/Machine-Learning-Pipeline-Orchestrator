  ...
  const send_data = () => {
    ...
    fetch(`${BASE_URL}:${CORE_PORT}/api/getworkerport/?token=${token}`)
      ...
      .then(response => { 
        ...
        // Create FormData to send JSON and CSV as multipart
        const formData = new FormData();
        
        // Append job specification as JSON string
        formData.append("job_specification", JSON.stringify(jobSpec));
        
        // Append the CSV file (passed as binary content)
        formData.append("data_file", dataFile); // Send the actual file here
