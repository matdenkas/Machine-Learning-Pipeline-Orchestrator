...
const PipelineDesigner = () => {
  ...
  const addNode = (nodeType) => {
    ...
    setNodeConfigs((prev) => ({ ...prev, [newNode.id]: getDefaultConfig(nodeType) })); // Initialize config
  };
  ...
  const handleDeleteNode = (nodeId) => {
    ...
    setNodeConfigs((prev) => {
      ...
      delete newConfigs[nodeId]; // Remove config for deleted node
      return newConfigs;
    });
  };
  ...
  const handleNodeClick = (event, node) => {
    ...
    if (!nodeConfigs[node.id]) {
      setNodeConfigs((prev) => ({ ...prev, [node.id]: getDefaultConfig(node.type) })); // Load default configuration
    }
  };
  ...
  const generateJson = () => {
    ...
    nodes.forEach((node) => {
      const config = nodeConfigs[node.id]; // Get the configuration for the node
      if (node.type === 'input') {
        json.nodes.push({
          id: node.id,
          type: 'input',
          data: { file: config.file ? config.file.name : 'No file selected' },
        });
      }
        ...
        <div style={{ width: '300px', padding: '20px' }}>
          ...
          {selectedNode && (
            <div>
                ...
                <>
                  ...
                  <input
                    ...
                    onChange={(e) => setNodeConfigs((prev) => ({
                      ...prev,
                      [selectedNode.id]: { ...prev[selectedNode.id], file: e.target.files[0] }
                    }))}
                  />
                </>
              )}
                    ...
                    onChange={(e) => {
                      ...
                      setNodeConfigs((prev) => ({
                        ...
                        [selectedNode.id]: {
                          ...
                          params: models[selectedAlgorithm], // Load default params for the selected model
                        }
                      }));
                    }}
                  >
              ...
              {selectedNode.type === 'output' && <p>Output Node: No configuration required.</p>}
            </div>
          )}
